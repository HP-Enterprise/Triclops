...
public class AESUpDataHandler extends ChannelInboundHandlerAdapter {
    ...
    private byte[] notEncryptApplication={0x11};//全部明文传输，无需加解密的业务（电检）

    public AESUpDataHandler(SocketRedis s,ConcurrentHashMap<String,String> connections,DataTool dataTool){
        this.socketRedis=s;
    ...
    public  byte[] upDatafilter(byte[]  content,Channel ch) {
        ...
        byte[] re=new byte[content.length];//原文
        byte[] withOutCheckSum=new byte[content.length-1];//原文
        ...
        if(dataType==0x12){//激活报文内容使用密钥（IMEI）
            aesKey=imei;
        }else if(dataType==0x13){//注册报文使用密钥（Serial Number+VIN）
            aesKey=imei;//t通过imei找到密钥
        }else{
        ...
        if(dataType!=0x11){//除了电检业务 其他都需要加解密
            ...
            withOutCheckSum=dataTool.getBytesFromByteBuf(tmp);//without checkSum
