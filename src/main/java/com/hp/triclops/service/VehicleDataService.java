...
public class VehicleDataService {
    ...
    @Value("${com.hp.remoteControl.wakeUpWaitSeconds}")
    private int wakeUpWaitSeconds;//唤醒等待时间，超过即为唤醒失败
    @Value("${com.hp.remoteControl.remoteControlTimeOut}")
    private int remoteControlTimeOut;//远程控制API返回超时秒数（唤醒成功后等待时间）
    @Value("${com.hp.remoteControl.remoteSettingTimeOut}")
    private int remoteSettingTimeOut;//远程设置超时秒数(唤醒成功后等待时间)
    ...
    public RemoteControl handleRemoteControl(int uid,long id,String vin,RemoteControlBody remoteControlBody,boolean isRefCommand){
       ...
       /* if(!initCheck(vin,rc.getControlType())){
        ...
        if(hasConnection(vin)){
            ...
            rc.setRemark("远程指令未执行,由于网络信号质量差或电瓶电量低，车联网远程控制功能暂时无法使用");
            rc.setRemarkEn("remote command not implemented, due to poor network signal quality or low battery power, car networking remote control function temporarily unavailable");
            rc.setStatus((short)0);
            remoteControlRepository.save(rc);
