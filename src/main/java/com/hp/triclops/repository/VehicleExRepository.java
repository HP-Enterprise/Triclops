...
public interface VehicleExRepository extends CrudRepository<VehicleEx, String> {
    ...
    @Query("select Ve.id from VehicleEx Ve where (?1 is null or Ve.vin like ?1) and (?2 is null or Ve.tboxsn like ?2)")
    List<Integer> findByVinAndTboxsn(String vin, String tboxsn);
    ...
    @Query("select Ve from VehicleEx Ve " +
           ...
           "and (?6 is null or Ve.regTime >= ?6) " +
           "and (?7 is null or Ve.regTime <= ?7) " +
